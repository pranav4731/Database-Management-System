# -*- coding: utf-8 -*-
"""
Created on Sat May 30 15:01:12 2020
    ...
    def __init__(self,root):
        ...
        def update():
            if(len(StdID.get())!=0):
                stdDatabase_Backend.deleteRec(sd[0])
            if(len(StdID.get())!=0):
                stdDatabase_Backend.deleteRec(StdID.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(), Address.get(), Mobile.get())
                studentlist.delete(0,END)
                studentlist.insert(END,StdID.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(), Address.get(), Mobile.get())
        ...
        #Title Frame(will be inside the main frame)
        ...
        DataFrame = Frame(MainFrame, bd=1, width=1300, height=50, padx=20, pady= 20, bg="cadet blue", 
                         relief=RIDGE)
        DataFrame.pack(side=BOTTOM)


        DataFrameLEFT = LabelFrame(MainFrame, bd=1, width=1000, height=600, padx=20, bg="Ghost White", 
                                   font=('arial', 20, 'bold'), text = "Student Info\n",
                         relief=RIDGE)
        DataFrameLEFT.pack(side=LEFT)


        DataFrameRIGHT = LabelFrame(MainFrame, bd=1, width=450, height=300, padx=31, pady= 3, bg="Ghost White",
